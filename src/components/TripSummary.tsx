...
interface Props {
  booking: OriginalBooking | undefined;
...
export function TripSummary({ booking, query, expanded, onToggle, onQueryChange }: Props) {
  const originTimeZone = booking ? findAirport(booking.origin)?.timeZone ?? "UTC" : "UTC";
  const destinationTimeZone = booking ? findAirport(booking.destination)?.timeZone ?? "UTC" : "UTC";
  const showForm = !booking || expanded;
  return (
    <section className={`trip-summary ${booking ? "" : "trip-summary--empty"}`} aria-labelledby="original-trip-title">
      <h2 id="original-trip-title">{booking ? "Original trip" : "Enter your original trip"}</h2>
      {booking ? (
        <>
      ...
      ) : <p className="trip-summary__intro">Add the cancelled or changed flight you already booked. Flywise will use it as the baseline for replacement matches.</p>}
      {showForm ? (
        <div className="original-trip-form" aria-label="Original flight details">
          ...
          <label>Original fare (USD)<input aria-label="Original fare" type="number" min={0} value={query.originalFare || ""} onChange={(event) => onQueryChange({ ...query, originalFare: Number(event.target.value) })} placeholder="Optional" /></label>
